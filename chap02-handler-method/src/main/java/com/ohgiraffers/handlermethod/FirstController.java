...
public class FirstController {
    ...
    * */
    @PostMapping("regist")
    public String registMenu(ModelAndView model, WebRequest request){
        /* WebRequst 객체의 getParameter 등의 메서드를 통해 클라이언트로 부터 전달 된 파라미터를 가져올 수 있다.*/
     ...
     * */
    @PostMapping("modify")
    public String modifyMenuPrice(Model model,@RequestParam(name = "modifyName1") String modifyName,
                                  @RequestParam(name = "modifyPrice1",required = false) int modifyPrice){
    ...
    * */
    ...
    public String modifyMenu(Model model, @RequestParam Map<String, String> parameters){
        ...
        String message =  "매뉴의 이름을 " + modifyMenu + "(으)로, 가격을 " + modifyPrice+"원 으로 변경하였습니다.";
        System.out.println(message);
    ...
    * 호ㅘ면에서 출력해보면 모델에 담겨진 값을 확인할 수 있다.
    *
    ...
    * 클래스레벨에 @SessionAttributes 어놑테이션을 이용하여 세션에 값을 담을 key를 설정해 두면
    ...
    * @CookieValue를 이용해서 쿠기 정보도 쉽게 불러올 수 있다.
    * */
    @PostMapping("body")
